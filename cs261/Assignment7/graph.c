...
int DFS(Graph* g, Vertex* source, Vertex* destination)
{
	...
	struct Vertex *current = malloc(sizeof(struct Vertex));
		...
		if (current == destination){
			...
			for (i = 0; i < current->numNeighbors; ++i){
				...
				if (!current->neighbors[i]->isVisited){
					...
					current->neighbors[i]->isVisited = 1;
				}
			}
...
int BFS(Graph* g, Vertex* source, Vertex* destination)
{
	...
	addFrontCirListDeque(list, source);

	while (!isEmptyCirListDeque(list)) {

		current = frontCirListDeque(list);
		...
		if (current == destination) {
			...
			for (i = 0; i < current->numNeighbors; ++i){
				...
				if (!current->neighbors[i]->isVisited){
					...
					addBackCirListDeque(list, current->neighbors[i]);
					current->neighbors[i]->isVisited = 1;
				}
			}
			current->isVisited = 1;
